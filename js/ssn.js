...
function reqListener () { //run when file is parsed
    ...
    ssndetails += "<br><br> If no data is available, or many dates are near 2011, consider SSN Randominazation.  The Social Security Administration (SSA) changed the way Social Security Numbers (SSNs) are issued on June 25, 2011. <br>It eliminated the geographical significance of the first three digits of the SSN, referred to as the area number, by no longer allocating the area numbers for assignment to individuals in specific states.<br>It eliminated the significance of the highest group number and, as a result, the High Group List is frozen in time and can only be used to see the area and group numbers SSA issued prior to the randomization implementation date.<br>Previously unassigned area numbers were introduced for assignment excluding area numbers 000, 666 and 900-999.<br>For blocks fully assigned prior to 2011 the state code of issuance are still valid. For blocks that were partially assigned, the lower numbers encode the state, and those issued after 2011 are random state.  For blocks that have very low death count and date after 2011 - then these blocks should be considered part of the random pool.<br>";
