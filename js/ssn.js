...
function reqListener () { //run when file is parsed
    ...
    var ssndetails = '<h3>SSNs in Block ' + num + '-xxxx </h3> For U.S. individuals with a Social Security Number from this block, the following analysis gives you an idea of how old the person is, what state it was issued in, when it was issued, and of those that have a SSN from this block, what the death statistics are.<br><br>';
    if (statecode != ""){ssndetails += "<img src=../img/s/"+statecode+".png width=25%><br><br>";}
    ...
    if(parselineresult[7] != ""){ssndetails += "VER. Issuing State prior to 2011: " + parselineresult[7] + " is " + '<a href="articles/' +statelong + '.html">' + statelong + '</a><br>';}
